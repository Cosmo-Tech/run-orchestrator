        ...
        if self.status == "Ready":
            if self.skipped:
                ...
                for input_name, input_config in self.inputs.items():
                    ...
                    if value is None and "defaultValue" in input_config:
                        ...
                        if input_config.get("hidden", False):
                            LOGGER.debug(T("csm-orc.logs.step.input.default_value_hidden").format(
                                step_id=self.id,
                                input=input_name
                            ))
                        else:
                            LOGGER.debug(T("csm-orc.logs.step.input.default_value").format(
                                step_id=self.id,
                                input=input_name,
                                value=value
                            ))
                    ...
                    if value is not None:
                        ...
                        raise ValueError(T("csm-orc.logs.step.input.missing_required").format(
                            step_id=self.id,
                            input=input_name
                        ))
                ...
                try:
                    ...
                    for output_name, output_config in self.outputs.items():
                        if "defaultValue" in output_config:
                            ...
                            if output_config.get("hidden", False):
                                LOGGER.debug(T("csm-orc.logs.step.output.default_value_hidden").format(
                                    step_id=self.id,
                                    output=output_name
                                ))
                            else:
                                LOGGER.debug(T("csm-orc.logs.step.output.default_value").format(
                                    step_id=self.id,
                                    output=output_name,
                                    value=output_config["defaultValue"]
                                ))
                    ...
                    LOGGER.debug(T("csm-orc.logs.step.output.captured_values_header").format(
                        step_id=self.id
                    ))
                    for output_name, value in self.captured_output.items():
                        if output_name in self.outputs and self.outputs[output_name].get("hidden", False):
                            LOGGER.debug(T("csm-orc.logs.step.output.captured_hidden").format(
                                output=output_name
                            ))
                        else:
                            LOGGER.debug(T("csm-orc.logs.step.output.captured_value").format(
                                output=output_name,
                                value=value
                            ))
                    ...
                    missing_outputs = []
                    for output_name, output_config in self.outputs.items():
                        if (output_name not in self.captured_output and 
                            not output_config.get("optional", False) and 
                            "defaultValue" not in output_config):
                            missing_outputs.append(output_name)
                            LOGGER.debug(T("csm-orc.logs.step.output.missing_value").format(
                                step_id=self.id,
                                output=output_name
                            ))
                    ...
                    LOGGER.info(T("csm-orc.logs.step.done_running").format(step_type=step_type, step_id=self.display_id))
                    self.status = "Done"
                    
                except subprocess.CalledProcessError as e:
                    LOGGER.error(T("csm-orc.logs.step.error_during").format(step_type=step_type, step_id=self.display_id))
                    LOGGER.error(str(e))
