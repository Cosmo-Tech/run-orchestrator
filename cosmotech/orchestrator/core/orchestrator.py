...
class FileLoader:
    ...
    def load_step(container, override: bool = False, **step) -> Step:
        _id = step.get('id')
        LOGGER.debug(T("csm-orc.logs.orchestrator.loading_step").format(
            id=f"{_id} of type Step"))
...
class Orchestrator(metaclass=Singleton):
    ...
    def _load_from_json_content(
        ...
        for _step, _node in _steps.values():
            if _step.precedents:
                LOGGER.debug(T("csm-orc.logs.orchestrator.dependencies.header").format(
                    step_id=_step.id))
            else:
                LOGGER.debug(T("csm-orc.logs.orchestrator.dependencies.no_dependencies").format(
                    step_id=_step.id))
        ...
        if display_env:
            ...
            for k, v in sorted(_env.items(), key=lambda a: a[0]):
                ...
                LOGGER.info(T("csm-orc.logs.orchestrator.environment.variable").format(
